...
function $viewsDelimiters(openChars, closeChars, link) {
	...
	//          [tag    (followed by / space or })  or cvtr+colon or views or code] followed by space+params then convertBack?
	...
	// Default:  bind     tagName         cvt   cln views code    params            slash   bind2         closeBlk  comment
	//      /(?:{(\^)?{(?:(\w+(?=[\/\s}]))|(\w+)?(:)|(>)|(\*))\s*((?:[^}]|}(?!}))*?)(\/)?|{(\^)?{(?:(?:\/(\w+))\s*|!--[\s\S]*?--))}}

	$sub.rTmpl = new RegExp("<.*>|([^\\\\]|^)[{}]|" + openChars + ".*" + closeChars);
	// $sub.rTmpl looks for views tags or { or } char not preceded by \\, or JsRender tags {{xxx}}. Each of these strings are considered
...
function compileTmpl(name, tmpl, parentTmpl, options) {
	...
	function lookupTemplate(value) {
		...
		if (("" + value === value) || value.nodeType > 0 && (elem = value)) {
			if (!elem) {
				if (/^\.\/[^\\:*?"<>]*$/.test(value)) {
					// tmpl="./some/file.views"
					// If the template is not named, use "./some/file.views" as name.
					if (tmpl = $templates[name = name || value]) {
						value = tmpl;
					} else {
						// BROWSER-SPECIFIC CODE (not on Node.js):
						// Look for server-generated script block with id "./some/file.views"
...
/*
     ...
     bind     tagName         cvt   cln views code    params            slash   bind2         closeBlk  comment
/(?:{(\^)?{(?:(\w+(?=[\/\s}]))|(\w+)?(:)|(>)|(\*))\s*((?:[^}]|}(?!}))*?)(\/)?|{(\^)?{(?:(?:\/(\w+))\s*|!--[\s\S]*?--))}}/g
...
function buildCode(ast, tmpl, isLinkExpr) {
	...
	code = "// " + tmplName
		...
		+ (hasEncoder ? ",h=j._html" : "")           // views converter
		+ (isLinkExpr ? ";\n" : ',ret=""\n')
		...
		if (setter = fnSetters[attr]) {
			if (attr === HTML) {
				...
				if (tag && tag._.inline) {
					...
					// data-linked value targeting innerHTML: data-link="views{:expr}" or contenteditable="true"
...
function defaultAttr(elem, to, linkGetVal) {
	...
	var nodeName = elem.nodeName.toLowerCase(),
		attr =
			...
			|| elem.contentEditable === TRUE && {to: HTML, from: HTML}; // Or if contentEditable set to "true" set attr to "views"
	return attr
		? (to
		...
		while (l--) {
			...
			if ("" + tmplOrLinkExpr === tmplOrLinkExpr) {
				...
				if (tmplOrLinkExpr.markup !== undefined) {
					...
					// TODO Consider finding a way to bind data (link) within template without views being different for each view, the HTML can
...
//}
	...
	if (html != undefined) {
		//================ Insert views into DOM using documentFragments (and wrapping HTML appropriately). ================
		// Also convert markers to DOM annotations, based on content model.
		// Corresponds to nextNode ? $(nextNode).before(views) : $(parentNode).views(views);
...
//			if (!!oldElCnt !== !!elCnt) {
//				error("Parse: " + views); // Parse error. Content not well-formed?
//			}
		...
		// Go to views and back, then peel off extra wrappers
		// Corresponds to jQuery $(nextNode).before(views) or $(parentNode).views(views);
...
//			if (!(linkTags = links[linkMarkup])) {
		...
		while (tokens = linkExpressions.shift()) {
			...
			// tokens: [all, attr, bindOnly, tagExpr, tagName, converter, colon, views, comment, code, params]
...
$extend($tags["for"], {
	...
	onArrayChange: function(ev, eventArgs, tagCtx, linkCtx) {
		...
		if (tag._.noVws // Child views not supported because target is not views - e.g. data-link="title{for ...}"
...
//$views.utility = {
//	validate: function(views) {
//		try {
//			topView.link(undefined, document.createElement("div"), undefined, undefined, views, undefined, undefined, 1);
//		}
//		catch (e) {
//			return e.message;
//		}
...
$viewsSettings.advanced({
	...
	// TODO investigate whether more recent jQuery implementation using wrapMap in domManip/$().views() etc. is better optimized now...
	_wm: {
